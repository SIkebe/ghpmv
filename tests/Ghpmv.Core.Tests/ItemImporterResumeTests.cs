...
public class ItemImporterResumeTests
{
    ...
    public async Task Ambiguous_item_create_is_reconciled_without_resending(bool draft)
    {
        ...
        var directory = Directory.CreateTempSubdirectory("ghpmv-resume-").FullName;
        try
        {
    ...
    {
        var cancellationToken = TestContext.Current.CancellationToken;
        var directory = Directory.CreateTempSubdirectory("ghpmv-field-partial-").FullName;
        try
        {
            using var handler = new StageResumeHandler("field", failureAttempt: 2);
            using var client = new GitHubGraphQLClient("token", baseUrl: null, handler, (_, _) => Task.CompletedTask);
    ...
    {
        var cancellationToken = TestContext.Current.CancellationToken;
        var directory = Directory.CreateTempSubdirectory("ghpmv-archive-reconcile-").FullName;
        try
        {
    ...
    {
        var cancellationToken = TestContext.Current.CancellationToken;
        var directory = Directory.CreateTempSubdirectory("ghpmv-position-chain-").FullName;
        try
        {
    ...
    {
        var cancellationToken = TestContext.Current.CancellationToken;
        var directory = Directory.CreateTempSubdirectory("ghpmv-mapping-resume-").FullName;
        try
        {
            using var handler = new StageResumeHandler(failedStage: "");
            using var client = new GitHubGraphQLClient("token", baseUrl: null, handler, (_, _) => Task.CompletedTask);
