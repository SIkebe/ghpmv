...
public class CollaboratorImportTests
{
    ...
    public async Task Import_applies_collaborators_and_linked_repositories_and_warns_on_unresolvable_entries()
    {
        ...
        var teamSlug = "ghpmv-collab-test-" + Guid.NewGuid().ToString("N")[..8];
        var missingUser = "ghpmv-missing-user-" + Guid.NewGuid().ToString("N")[..8];
        var missingRepo = SourceOrg + "/ghpmv-missing-repo-" + Guid.NewGuid().ToString("N")[..8];
    ...
    private static HttpClient CreateRestClient()
    {
        ...
        http.DefaultRequestHeaders.UserAgent.ParseAdd("ghpmv-tests");
        return http;
    }
