...
| Text / Number / Single-select field defaults | ✅ with browser automation | Public REST/GraphQL APIs do not expose defaults. Browser-assisted export/import/verify preserves configured and cleared defaults; Single-select defaults are remapped by option name. Defaults are applied only after every source item imports successfully, so existing or skipped items cannot inherit them. API-only import cannot guarantee an existing target default when Single-select option reconciliation removes its option. Date defaults are not supported by GitHub. |
