...
| ブラウザー E2E テスト | `tests/Ghpmv.Browser.Tests` の単一共有 E2E シナリオ | 一つの source export / target Project / import / BrowserSession で collaborator、View、Workflow の round-trip と drift 検出を検証する。`BrowserE2eArchitectureTests` が独立 E2E entry point の追加を禁止する。 | `GHPMV_BROWSER_STATE`、`GHPMV_TEST_TOKEN`、source/target fixture org。未設定時は skip。 | `src/Ghpmv.Core/Browser` 変更時とリリース前に手動実行。scheduled/nightly は未実装。 |
