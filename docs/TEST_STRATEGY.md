...
| 実 API 統合テスト | `tests/Ghpmv.Integration.Tests` | GraphQL 接続、export/import/verify、View の API 設定、organization Issue Field lifecycle、collaborator/repository/Team link、user-owned project、item resume/relink を実 GitHub API で検証する。 | `GHPMV_TEST_TOKEN` と fixture 用 org/project 変数。token owner は source/target 両 organization で disposable Team と private repository を作成・削除できること。classic PAT は `admin:org` と `repo`、fine-grained PAT は **Members: Read and write** と repository **Administration: Read and write**（All repositories）が必要。ローカルと資格情報のない PR では skip。 | secrets が使えるリポジトリ CI の .NET 変更 PR、毎日の scheduled run、手動実行、API 変更時のローカル検証。 |
...
| 変数 | 使用箇所 | 意味 |
|---|---|---|
| `GHPMV_TEST_TOKEN` | 統合テスト、browser E2E | fixture org に対して SSO authorization 済みの token。integration suiteではsource/target両organizationのdisposable Teamとprivate repositoryを作成・削除できること。classic PATは`admin:org`と`repo`、fine-grained PATは**Members: Read and write**とrepository **Administration: Read and write**（All repositories）が必要。単一tokenで両orgを操作するため、cross-organization構成では通常classic PATを使う。 |
