...
8. `Truncate titles` / `Show date fields` は親 View menu の direct `menuitemcheckbox` で、状態は `aria-checked` に保持される。2026-08-19 の discovery では control の存在と checkbox state を確認したが、menu text に current value は表示されなかった。save/reload persistence と per-view isolation は当時の記録だけでは未確認のため、ghpmv は property ごとに取得不能を nullable + warning として扱い、import 後の reload read-back と `Fixture Roadmap` だけを変更する mixed-state drift E2E を必須 gate にする
