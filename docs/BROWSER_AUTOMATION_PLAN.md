...
| タブの並び順 | GraphQL `views(orderBy: {field: POSITION})` | **UI**(タブの drag & drop) | `ViewSnapshot.tabPosition` は schema v1 の nullable additive field |
...
| V-10 | View の name / タブ並び順 | GraphQL POSITION で読み、browser-assisted import で最小 D&D を適用 |
...
EnrichView(spec, targetViewNumber):
 ...
 9. 全 View 設定の適用後、target の GraphQL POSITION 順と snapshot 順から最小移動計画を作り、必要なタブだけ drag-and-drop
...
作成順序: `tabPosition` がある snapshot は **POSITION 昇順**、旧 snapshot は view number 昇順で作成する。デフォルトで作られる "View 1" は snapshot の先頭 View で上書き(rename + 設定)して消費する。API-only import は現在順との差分を warning にし、browser-assisted import は View 設定適用後に target 順を GraphQL で再取得して修復する。
...
1. **API で読める項目**: GraphQL で対象 view を `views(first:50, orderBy:{field:POSITION})` から number 一致で取得し、`tabPosition / layout / filter / groupByFields / sortByFields / verticalGroupByFields / visibleFields` を spec と比較
...
| B10 | View tab POSITION export / verify + browser D&D import | 完了。旧 snapshot の null は比較・修復対象外 |
