...
When an import fails, `ghpmv` writes `import-error.json` in the snapshot directory and prints its path to stderr. The report contains the requested and actual target Project context, the failure stage, the latest 200 progress messages, exception and inner-exception stack traces, GraphQL error types, and HTTP status codes. Raw GraphQL error responses, tokens, and mutation variables are not written to the report. A later failure in the same snapshot directory replaces the previous report; a successful completed import removes it, while a skipped import leaves the previous report available for investigation.
