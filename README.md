...
This is an **API failure diagnostic boundary, not an anonymity guarantee for all output**. Snapshots, mappings, verification differences, project/item progress, pending-operation/resume payloads, and recovery targets necessarily contain business names, IDs, URLs, titles, or content. `import-error.json` also retains target context and recent progress. Browser automation warnings/exceptions can contain Playwright call logs, selectors, page URLs, and business details; this flag neither collects nor sanitizes those browser details. Review all such artifacts before sharing. Migration payloads and stdout result formats are intentionally unchanged.
